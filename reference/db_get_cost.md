...
downloads are billed per gigabyte, previews are not. Mirrors
`metadata.get_cost`. The quote already reflects any plan discount, so it
is not simply billable size times unit price.
...
A single number: the quoted cost in USD.
